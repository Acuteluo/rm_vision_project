...
#include <CameraApi.h>    // 迈德威视 SDK 主头文件

// ROS
#include <camera_info_manager/camera_info_manager.hpp>  // ROS 相机信息管理
#include <image_transport/image_transport.hpp>  // ROS 图像传输（支持压缩/原始）
#include <rclcpp/logging.hpp>  // ROS2 日志
#include <rclcpp/rclcpp.hpp>
#include <sensor_msgs/msg/camera_info.hpp>  // 相机信息消息类型
...
public:
  explicit MVCameraNode(const rclcpp::NodeOptions & options) : Node("mv_camera", options)
  {
    ...
    camera_pub_ = image_transport::create_camera_publisher(this, "image_raw", qos); // 声明相机图像发布话题 image_raw

    // Load camera info from "camera_info.yaml"
    camera_name_ = this->declare_parameter("camera_name", "mv_camera"); // 这个相机名称要和 config下的相机信息一样
    camera_info_manager_ =
      std::make_unique<camera_info_manager::CameraInfoManager>(this, camera_name_);
    auto camera_info_url = this->declare_parameter( // 加载相机信息
        // package://下面是包名名称
      "camera_info_url", "package://my_mindvision_camera/config/camera_info.yaml");
    ...
    capture_thread_ = std::thread{[this]() -> void {
      RCLCPP_INFO_ONCE(this->get_logger(), "-------  Publishing image! 相机已经启动! -------");

      image_msg_.header.frame_id = "camera_optical_frame";
      image_msg_.encoding = "rgb8"; // 发布的图像编码

      while (rclcpp::ok()) {
        int status = CameraGetImageBuffer(h_camera_, &s_frame_info_, &pby_buffer_, 1000); // 超时时间 1s
        if (status == CAMERA_STATUS_SUCCESS) { // 成功则调用 CameraImageProcess 将原始 Bayer 数据转为 RGB8，存入 image_msg_.data
        ...
        // 失败计数：如果连续 5 次失败，则 rclcpp::shutdown()
        if (fail_conut_ > 5) {
  ...
  ~MVCameraNode() override
  {
    ...
    RCLCPP_INFO(this->get_logger(), "-------  Camera node destroyed! 相机节点已被退出  -------");
  }
...
private:
  void declareParameters()
  {
    ...
    double exposure_time = this->declare_parameter("exposure_time", 5000, param_desc); // 设置曝光时间
    CameraSetExposureTime(h_camera_, exposure_time);
    RCLCPP_INFO(this->get_logger(), "曝光时间 Exposure time = %f", exposure_time);
    ...
    // Gamma 较低的伽马值（如0.45）会使暗部细节更明显，但整体对比度降低；较高的伽马值（如1.8）会增强对比度，但暗部可能过暗。一般1.0-2.2
