...
void YoloArmor::SetArmorplateSize() 
{
    ...
    double armorplate_width = is_big ? 0.225 : 0.135;
...
void YoloArmor::perspectiveNPoint() 
{
    ...
    if (corners.size() != 4 || vertice_cv_.size() != 4) 
    {
        this->pnp_success = false;
        return;
    }

    // 1. 执行 OpenCV 的 solvePnP (极高精度的 2D 物理角点，直接 IPPE)
    this->pnp_success = cv::solvePnP(vertice_cv_, corners, K, D, r_cv_, t_cv_, false, cv::SOLVEPNP_IPPE);
    ...
    this->t_vec = P_.transpose() * t_cv_eigen; // t_flu = P^(-1) * t_cv
    ...
    this->R = P_.transpose() * R_cv_eigen * P_; // R_flu = P^(-1) * R_cv * P

    // ==========================================================
    // 3. 供人类调试和下位机使用的极坐标 (Yaw, Pitch, Distance)
    // ==========================================================
    double X = this->t_vec(0); // 向前
    double Y = this->t_vec(1); // 向左
    double Z = this->t_vec(2); // 向上

    this->t_distance = std::sqrt(X * X + Y * Y + Z * Z);
...
void YoloArmor::DrawAndPrintInfo(cv::Mat& img_show)
{
    ...
    for (int i = 0; i < 4; i++) 
    {
        cv::line(img_show, corners[i], corners[(i + 1) % 4], cv::Scalar(0, 255, 0), 2);
        // 画角点序号，方便验证顺序 (0左上, 1左下, 2右下, 3右上)
        cv::putText(img_show, std::to_string(i), corners[i], cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 128, 255), 2);
    }
    ...
    std::string info_dist = "Dist: " + std::to_string(t_distance).substr(0, 4) + "m";
    std::string info_pitch = "Pitch: " + std::to_string(t_pitch).substr(0, 5);
    std::string info_yaw = "Yaw: " + std::to_string(t_yaw).substr(0, 5);

    cv::putText(img_show, info_dist, cv::Point2f(box.x, box.y - 35), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 255), 2);
    cv::putText(img_show, info_pitch, cv::Point2f(box.x, box.y - 15), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 255), 2);
    cv::putText(img_show, info_yaw, cv::Point2f(box.x, box.y + box.height + 20), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(255, 255, 255), 2);
    
    // 4. 打印装甲板 ID 和类型
    std::string type_str = is_big ? "BIG" : "SMALL";
    cv::putText(img_show, "ID:" + std::to_string(armor_id) + " " + type_str, center, cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 255), 2);
}