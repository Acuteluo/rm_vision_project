...
def generate_launch_description():
    ...
    core_node = Node(
        ...
        name='core_node',
        parameters=[{'use_sensor_data_qos': True}],   # 添加这一行
        output='screen'
    )
    ...
    serial_driver_launch = Node(
        ...
        name='serial_driver',
        parameters=[os.path.join(get_package_share_directory('rm_serial_driver'), 'config', 'serial_driver.yaml')],
        output='screen'
    )
